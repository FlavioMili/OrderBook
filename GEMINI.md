# objective
I want to make this code much much faster, but first you need to make sure that the order generation is faster and that 
it only creates a wrong order every 1/50millions as per the configuration file.

please try to use pipelining to improve the speed and maybe take inspiration from the winnners of the one billion rows challenge to implement faster
code.

here is the winner code: 
``` Java
/**
